...
export class LoadPalladioCommand extends Command {
    ...
    private getModelFiles(): Promise<File[] | undefined> {
        ...
        input.type = "file";
        input.accept = ".pddc, .allocation, .nodecharacteristics, .repository, .resourceenvironment, .system, .usagemodel";
        input.multiple = true;
            ...
            input.onchange = () => {
                if (input.files && input.files.length === 7) {
                    ...
                    if (allFilesPresent) {
                        ...
                        reject("Please select one file of each required type: .pddc, .allocation, .nodecharacteristics, .repository, .resourceenvironment, .system, .usagemodel");
                    }
                } else {
