...
export class SimplifyNodeNamesCommand extends Command {
  ...
  constructor(@inject(TYPES.Action) _: Action, @inject(NodeNameRegistry) private readonly nodeNameRegistry: NodeNameRegistry, @inject(SETTINGS.SimplifyNodeNames) private readonly simplifyNodeNames: SimplifyNodeNames) {
    super()
  }
  
  execute(context: CommandExecutionContext): CommandReturn {
    this.iterate(context.root, (n) => this.simplifyNodeNames.get() ? this.nodeNameRegistry.setAnonymousName(n) : this.nodeNameRegistry.setPlainName(n))
    return context.root
  }
