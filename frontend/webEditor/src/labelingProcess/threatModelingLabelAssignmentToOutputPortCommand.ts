...
export class ThreatModelingLabelAssignmentToOutputPortCommand implements Command {
    ...
    execute(context: CommandExecutionContext): CommandReturn {
        ...
        if (!isThreatModelingLabelType(labelType) || !isThreatModelingLabelTypeValue(labelTypeValue)) {
            this.applyNewBehavior([`${this.previousBehavior}`, `set ${labelType.name}.${labelTypeValue.text}`]);
            return context.root;
        }

        const lines = this.previousBehavior.split("\n").map((line) => line.trim());
    ...
    redo(context: CommandExecutionContext): CommandReturn {
        if (!this.newBehavior || this.action.collisionMode === "askUser") return context.root;
    ...
    undo(context: CommandExecutionContext): CommandReturn {
        if (!this.previousBehavior || this.action.collisionMode === "askUser") return context.root;
    ...
    private handleOverwrite(
        ...
        for (const collision of collisions) {
            lines = removeLabelAssignment(lines, {
                labelType: collision.labelType,
                labelTypeValue: collision.labelTypeValue,
            });
        }
        lines = addLabelAssignment(lines, candidate, this.labelTypeRegistry);
        this.applyNewBehavior(lines);
    ...
    private applyNewBehavior(lines: string[]) {
        this.newBehavior = lines.filter((line) => line !== "").join("\n");
        this.action.element.setBehavior(this.newBehavior);
...
export function findCollisions(
    ...
    const collisions = new Map<
        ...
        { labelType: ThreatModelingLabelType; labelTypeValue: ThreatModelingLabelTypeValue }
    >();
    const computeCompositeKey = (labelType: ThreatModelingLabelType, labelTypeValue: ThreatModelingLabelTypeValue) =>
        ...
        if (line.trim() === `unset ${candidate.labelType.name}.${candidate.labelTypeValue.text}`) {
            ...
            for (let j = i; j >= 0; j--) {
                if (portBehavior[j].startsWith(`set`)) {
                    ...
                    const labelType = labelTypeRegistry
                        .getLabelTypes()
                        .find((labelType) => labelType.name === labelTypeName);
                    if (!labelType) continue;

                    const labelTypeValue = labelType.values.find(
                        (labelTypeValue) => labelTypeValue.text === labelTypeValueText,
                    );
                    ...
                    collisions.set(computeCompositeKey(labelType, labelTypeValue), { labelType, labelTypeValue });
                }
            }
        }
