...
export namespace BeginLabelingProcessAction {
    export function create(labelTypeRegistry: LabelTypeRegistry): LabelingProcessAction {
...
export namespace NextLabelingProcessAction {
    export function create(
        ...
        const pendingLabels = labelTypeRegistry
            .getAllLabelAssignments()
            .filter(
                (label) =>
                    !finishedLabels.some(
                        (finishedLabel) =>
                            finishedLabel.labelTypeId === label.labelTypeId &&
                            finishedLabel.labelTypeValueId === label.labelTypeValueId,
                    ),
            );
...
export class LabelingProcessCommand implements Command {
    ...
    highlightShapes(context: CommandExecutionContext) {
        ...
        const applyColorToOutputPort = (port: DfdOutputPortImpl) => {
            ...
            if (
                findOutputPortCollisions(behavior, { labelType, labelTypeValue }, this.labelTypeRegistry).length === 0
            ) {
                port.setColor(LabelingProcessUi.ASSIGNABLE_COLOR);
            } else {
