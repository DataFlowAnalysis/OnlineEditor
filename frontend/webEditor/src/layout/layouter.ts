...
export class DfdElkLayoutEngine extends ElkLayoutEngine {
    ...
    protected override transformLabel(slabel: SLabel, index: SModelIndex): ElkLabel {
        ...
        if (this.configurator.method === LayoutMethod.WRAPPING) {
            return e;
        }
        ...
        if (this.getBasicType(sshape) === "port") {
            ...
            if (sshape instanceof SChildElementImpl && isBoundsAware(sshape.parent)) {
                ...
                if (
                    ...
                    if (this.configurator.method === LayoutMethod.CIRCLES) {
                        if (elkShape.x <= 0)
                            // Left edge
...
export class CircleLayoutPostProcessor implements ILayoutPostprocessor {
    ...
    constructor(@inject(DfdLayoutConfigurator) private readonly configurator: DfdLayoutConfigurator) {}
